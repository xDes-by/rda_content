...
var selectTalant = (function(arg)
{
	return function()
	{
        ...
        // $.Msg(pan.available)
        if(pan.available && talantpanel.FindChildTraverse("button") && pid == portID){
            talantpanel.FindChildTraverse("button").visible = true;
...
(function() {
    ...
    if(Game.GetState() >= DOTA_GameState.DOTA_GAMERULES_STATE_PRE_GAME){
        ...
        FindDotaHudElement("SelectedHeroAbilitiesHitTargets").GetChild(4).visible = false;
        FindDotaHudElement("SelectedHeroAbilitiesHitTargets").GetChild(5).visible = false;
    }
})();
...
function setIconParam(data){
    for(var n in cat = ["str","agi","int","don"]){
        ...
        let level = n == 'don' ? data['donlevel'] : data['level'];
        ...
        }else if(data[cat[n]+12] == 0 && level >= 30 && (data[cat[n]+11] == 1 || data[cat[n]+10] == 1 || data[cat[n]+9] == 1) && talantpanel.FindChildTraverse(cat[n]+12)){
            // 3 == don
            if(n == 3 || data["str12"] == 0 && data["int12"] == 0 && data["agi12"] == 0){
