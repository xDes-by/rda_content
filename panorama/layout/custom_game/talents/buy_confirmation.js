...
function BuyButton_Event(name, currency, price, combinable, value, event_name){
    return  ()=>{
        Game.EmitSound("General.ButtonClick");
        if(ThrowAnError(currency, price) == false){
            BuyButton(name, currency, price, combinable, value, event_name);
            CONFIRM.buy_confirm_price.text = price;
        }
    }
}
function BuyButton(name, currency, price, combinable, value, event_name){
    ...
    CONFIRM.confirm_yes_panel.SetPanelEvent("onactivate", ()=>{
        ...
        GameEvents.SendCustomGameEventToServer(event_name, {name:name, amount:amount,currency:currency});
        HideConfirmPanel();
    })
}
function PrepareConfirm(name, currency, price, combinable, value){
    CONFIRM.buy_confirm_item_name.text = $.Localize("#"+name) 
