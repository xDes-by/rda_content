...
function CreatePetList(t){
    ...
    for(i in tier){
        ...
        for(let z = 0; z < Math.ceil(tier[i].length/6); z++){
            ...
            let pagePanel = $.CreatePanel("Panel", $("#pet_showcase"), "", {class:"showcase"})
            
            for(let j = z*6; j < 6+6*z; j++){
                CreatePetPanel(tier, pagePanel, i, j, page)
            }
            let p = Pets.PageTitle.length - 1
            let dot = $.CreatePanel("Button", $("#ItemPips"), "" ,{class:"PaginationButtonCustom"})
...
function CreatePetPanel(tier, pagePanel, i, j, pageNumber){
    ...
    let petPanel = $.CreatePanel("Panel", pagePanel, pan_id, {class:"showcase_pet"})
    ...
    return ()=>{
        ...
        if(Pets.all[i][j].combinable){
            ...
            for(let i = 1; i <= opCount; i++){
                $("#confirm_drop_down").AddOption($.CreatePanel("Label", $("#confirm_drop_down"), i, {text:i+$.Localize("#pet_pieces")}))
            }
...
var open_btn_panel = $.CreatePanel("Panel", FindDotaHudElement("HUDElements"), "", {style:"height: 100px;width: 100px;background-image: url('file://{images}/custom_game/pet/cat.png');background-size: contain; background-position: 50%;background-repeat: no-repeat;margin-left: 325px;tooltip-position: bottom;"})
