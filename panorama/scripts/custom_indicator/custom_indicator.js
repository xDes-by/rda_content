...
function Think_snapfire()
{
	...
	if (vectorTargetParticle_snapfire)
	{
	    ...
	    if ( (Abilities.GetLocalPlayerActiveAbility() != -1) && (Abilities.GetAbilityName(Abilities.GetLocalPlayerActiveAbility()) == "snapfire_scatterblast_custom") ) {
			...
			Particles.SetParticleControl(vectorTargetParticle_snapfire, 1, Vector_add(origin, Vector_mult(direction, (cast_range + (blast_width_end / 2) ))) );
...
Think_dawnbreaker()
var vectorTargetParticle_dawnbreaker;
var lastAbility_dawnbreaker = -1;

function Think_dawnbreaker()
{
	if (Abilities.GetLocalPlayerActiveAbility() != lastAbility_dawnbreaker) 
	{
		lastAbility_dawnbreaker = Abilities.GetLocalPlayerActiveAbility()
		if (vectorTargetParticle_dawnbreaker) {
			Particles.DestroyParticleEffect(vectorTargetParticle_dawnbreaker, true)
			vectorTargetParticle_dawnbreaker = undefined;
		}
		if ( (Abilities.GetLocalPlayerActiveAbility() != -1) && (Abilities.GetAbilityName(Abilities.GetLocalPlayerActiveAbility()) == "dawnbreaker_starbreaker_lua") ) {
			vectorTargetParticle_dawnbreaker = Particles.CreateParticle("particles/units/heroes/hero_dawnbreaker/hero_dawnbreaker_combo_strike_range_finder_aoe.vpcf", ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, Players.GetLocalPlayerPortraitUnit() );
		}
	}
	...
	if (vectorTargetParticle_dawnbreaker)
	{
	    ...
	    if ( (Abilities.GetLocalPlayerActiveAbility() != -1) && (Abilities.GetAbilityName(Abilities.GetLocalPlayerActiveAbility()) == "dawnbreaker_starbreaker_lua") ) {
            ...
            let origin = Entities.GetAbsOrigin( Players.GetLocalPlayerPortraitUnit() )
	    	let radius = Abilities.GetSpecialValueFor(Abilities.GetLocalPlayerActiveAbility(), "smash_radius");
	    	let speed = Abilities.GetSpecialValueFor(Abilities.GetLocalPlayerActiveAbility(), "movement_speed");
	    	let duration = Abilities.GetSpecialValueFor(Abilities.GetLocalPlayerActiveAbility(), "duration");
	    	let delta = Abilities.GetSpecialValueFor(Abilities.GetLocalPlayerActiveAbility(), "smash_distance_from_hero");

	    	let direction = Vector_normalize(Vector_sub(origin, worldPosition));
	    	let cast_range = Abilities.GetCastRange( Abilities.GetLocalPlayerActiveAbility() )

	    	Particles.SetParticleControl(vectorTargetParticle_dawnbreaker, 0, origin );
			Particles.SetParticleControl(vectorTargetParticle_dawnbreaker, 1, Vector_sub(origin, Vector_mult(direction,  speed * duration + delta)) );
			Particles.SetParticleControl(vectorTargetParticle_dawnbreaker, 6, [ radius, 0, 0 ] );
	    }
	}

    $.Schedule(1/144, Think_dawnbreaker)
}